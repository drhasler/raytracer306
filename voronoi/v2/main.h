...
std::vector<double> sample_uni1(const size_t N) {
    std::mt19937 rng(time(0) + 69);
    std::uniform_real_distribution<double> uni(0,pow(N,-1.5));

    std::vector<double> pts(N);
    std::generate(pts.begin(), pts.end(), [&]() { return uni(rng); });
    return pts;
}

std::vector<vec2> sample_uni2(const size_t N) {
